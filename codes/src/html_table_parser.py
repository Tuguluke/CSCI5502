    ...
    def parse_html_table(self, table):
        ...
        for row in rows:
            ...
            if len(cells) > 0:
                ...
                new_columns = [element.find(text=True)
                               for element in row.findAll("th")]
        ...
        if len(columns) == len(df_table.columns):
            ...
            warnings.warn(
                "Column titles do not match the number of columns for the table with id: " + table['id'])
        return df_table
